import { ChainId, Currency, Ether, NativeCurrency, Token, UNI_ADDRESSES, WETH9 } from '@synthra-swap/sdk-core'
...
export const USDC_UOMI = new Token(
  ChainId.UOMI,
...
export const WRAPPED_NATIVE_CURRENCY: { [chainId: number]: Token | undefined } = {
  ...
  [ChainId.UOMI]: new Token(ChainId.UOMI, '0x5FCa78E132dF589c1c799F906dC867124a2567b2', 18, 'WUOMI', 'Wrapped UOMI'),
...
export function isUomi(chainId: number): chainId is  ChainId.UOMI {
  return chainId === ChainId.UOMI
}
class UomiNativeCurrency extends NativeCurrency {
  equals(other: Currency): boolean {
    return other.isNative && other.chainId === this.chainId
  }

  get wrapped(): Token {
    if (!isUomi(this.chainId)) throw new Error('Not uomi')
    const wrapped = WRAPPED_NATIVE_CURRENCY[this.chainId]
    invariant(wrapped instanceof Token)
    return wrapped
  }
  ...
  public constructor(chainId: number) {
    if (!isUomi(chainId)) throw new Error('Not uomi')
    super(chainId, 18, 'UOMI', 'UOMI')
  }
}
...
export function nativeOnChain(chainId: number): NativeCurrency | Token {
  ...
  } else if (isUomi(chainId)) {
    nativeCurrency = new UomiNativeCurrency(chainId)
  } else {
...
export const TOKEN_SHORTHANDS: { [shorthand: string]: { [chainId in ChainId]?: string } } = {
  USDC: {
    ...
    [ChainId.UOMI]: USDC_UOMI.address,
  },
}
